...
describe('Link', () => {
	it('should render correctly', () => {
		const { getByText, getByRole } = render(<Link href="/" label="Teste" />)

		expect(getByRole('link')).toBeInTheDocument()
	})

	it('should accept custom class name', () => {
		const { getByRole } = render(<Link className="custom-class" href="/" label="Teste" />)

		expect(getByRole('link')).toHaveClass('custom-class')
	})

	it.each(variants)('renders correctly with variant %s', (variant) => {
		const { getByRole } = render(<Link href="/" label="Teste" variant={variant} />)
	...
	it.each(variants)('renders correctly with variant %s and active', (variant) => {
		const { getByRole } = render(<Link active href="/" label="Teste" variant={variant} />)
		const element = getByRole('link')
